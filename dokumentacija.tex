...
Cijeli sustav pokreće se jednom \texttt{docker-compose} naredbom. AWS infrastruktura simulira se lokalno putem LocalStacka, što eliminira potrebu za oblačnim računom u razvoju.

% ========================================================================
\section{Arhitektura sustava}

Svaka poslovna funkcija izdvojena je u zaseban servis. Servisi ne dijele bazu podataka ni memoriju -- jedina sprega je mrežna komunikacija. Svih šest servisa i dvije infrastrukturne komponente smješteni su na zajedničku Docker mrežu \texttt{rent\_network}, unutar koje se pronalaze putem Docker DNS rezolucije.
...
Prednost ovog pristupa je što poruke preživljavaju restart workera -- čak i ako Mail-worker padne, Booking-service nastavlja primati zahtjeve, a poruke u redu čekaju na obradu. Redis tu djeluje kao buffer koji apsorbira nagle skokove u prometu.
...
Pružena sučelja odgovaraju javnim API-jima pojedinih komponenti: \texttt{IAuth}, \texttt{IBooking} i \texttt{IDamage} su REST sučelja izložena FastAPI-jem, \texttt{ILocation} je gRPC sučelje definirano \texttt{gps.proto} datotekom, \texttt{IEmailQueue} je Redis list API koji dijele BookingService (producent) i MailWorker (konzument), a \texttt{IS3} je S3 sučelje koje LocalStack lokalno simulira.
